...
#PANOPOLY_FEATURES="panopoly_core panopoly_images panopoly_pages panopoly_theme panopoly_widgets"
...
for panopoly_feature in $PANOPOLY_FEATURES; do
  if [ x$FIRST = x1 ]; then
    ...
    $DRUSH $DRUSH_ARGS features-diff $panopoly_feature 2>&1 >/dev/null
	FIRST=0
  fi
